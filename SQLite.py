...
#         """Проверка на существование пользователя в БД"""
#         info = self.cursor.execute('SELECT * FROM users WHERE user_id=?', (self.user_id,))
#         if info.fetchone() is None:  # Если человека нет в бд
#             return False
#         else:  # Если есть человек в бд
#             return True
...
#         """Проверка на то, является ли пользователь админом"""
#         info = self.cursor.execute('SELECT * FROM users WHERE status=? and user_id=?', ('admin', self.user_id))
#         if info.fetchone() is None:  # Если пользователь не админ
#             return False
#         else:  # Если человек админ
#             return True
...
#         """Проверка на то, подписался ли пользователь на рассылку уведомлений"""
#         info = self.cursor.execute('SELECT * FROM users WHERE notification=? and user_id=?', ('yes', self.user_id))
#         if info.fetchone() is None:  # Если пользователь НЕ подписан на рассылку
#             return False
#         else:  # Если пользователь подписан на рассылку
#             return True
#
#     def db_table_val(self):
#         """Проверка на уникальность и добавление данных о пользователе в SQL"""
#         # Словарь вносимых в базу данных значений
#         list_user = {
#             'ID: ': self.user_id,
...
#                     return str(a) + str(b) + '\n'
#
#         if check_for_existence(self.user_id) is False:
#             sqlite_select_query = 'SELECT * from users where user_id'
...
#                                 '?, ?)',
#                                 (self.user_id, self.first_name, self.last_name, self.username))
#             self.sqlite_connection.commit()
#             self.cursor.close()
#             end_text = 'К боту подключился новый пользователь!\n' + data_user() + '\n'
#             Data.bot.send_message(chat_id=Data.list_admins.get('Никита'), text=end_text)
#             print(end_text)
#         elif check_for_existence(self.user_id) is True:
#             # обновление изменений данных о пользователе:
#             sqlite_update_query = 'UPDATE users set user_first_name = ?, user_last_name = ?, username = ? WHERE ' \
#                                   'user_id =' + str(self.user_id)
#             column_values = (self.first_name, self.last_name, self.username)
...
#         try:
#             try_message = 'Все данные о пользователе <' + get_user_info(self.user_id) + '> успешно удалены из БД!'
#             print(try_message)
#             Other_function.logging_event('info', try_message)
#             sql_delete_query = 'DELETE from users where user_id = ' + str(self.user_id)
...
#         try:
#             sql_select_query = 'SELECT * FROM users WHERE user_id=?'
#             self.cursor.execute(sql_select_query, (self.user_id,))
#             records = self.cursor.fetchall()
#             for row in records:
#                 user_id_SQL = row[1]
#                 user_first_name_SQL = row[2]
...
#                 username_SQL = row[4]
#
#                 if self.user_id == user_id_SQL or \
#                         self.first_name != user_first_name_SQL or \
#                         self.last_name != user_last_name_SQL or \
#                         self.username != username_SQL:
#                     self.db_table_val()
#             self.cursor.close()
#         except sqlite3.Error as error:
#             print("Ошибка при работе с SQLite: ", error)
